...
# Have not figured out how to update initial launch post-deployment.  Need more research.
  ...
  depends_on = [
    aws_lambda_function.warmasglambda,
...
/*  
resource "aws_autoscaling_policy" "byol_scale_in" {
  name                   = "${random_id.deployment_id.hex}_byol_asg_scale_in"
...
resource "aws_cloudwatch_metric_alarm" "alarm_low" {
  alarm_name          = "${random_id.deployment_id.hex}_cloudwatch_alarm_low"
  alarm_actions       = [aws_autoscaling_policy.byol_scale_in.arn ]
  ...
  period              = "60"
 statistic           = "Average"
  threshold           = var.cloudwatch_threshold_low_alarm
  actions_enabled     = "true"
  unit                = "Count"
}

resource "aws_autoscaling_policy" "byol_scale_out" {
  name                   = "${random_id.deployment_id.hex}_byol_asg_scale_out"
  policy_type            = "SimpleScaling"
#  metric_aggregation_type = "Average"
...
resource "aws_cloudwatch_metric_alarm" "alarm_high" {
  alarm_name          = "${random_id.deployment_id.hex}_cloudwatch_alarm_high"
  alarm_actions       = [aws_autoscaling_policy.byol_scale_out.arn ]
  ...
  period              = "60"
  statistic           = "Average"
  threshold           = var.cloudwatch_threshold_high_alarm
  actions_enabled     = "true"
  unit                = "Count"
}
*/

data "archive_file" "warmasglambda_zip" {
  type = "zip"
  output_path = "warmasglambda.zip"
  source_dir = "./code/"
  depends_on = [
...
resource "aws_lambda_function" "addniclambda" {
  function_name = "addniclambda"
  runtime = "python3.9"
  filename = "${data.archive_file.warmasglambda_zip.output_path}"
  handler = "addniclambda.lambda_handler"
  role = aws_iam_role.warmasglambda_role.arn
  timeout = 900
  source_code_hash = "${data.archive_file.warmasglambda_zip.output_base64sha256}"
 vpc_config {
...
resource "aws_lambda_function" "warmasglambda" {
  function_name = "warmasglambda"
  runtime = "python3.9"
  filename = "${data.archive_file.warmasglambda_zip.output_path}"
  handler = "warmasglambda.lambda_handler"
  role = aws_iam_role.warmasglambda_role.arn
  timeout = 900
  source_code_hash = "${data.archive_file.warmasglambda_zip.output_base64sha256}"
 vpc_config {
...
resource aws_iam_role "warmasglambda_role" {
  name = "warmasglambda_role"
...
resource aws_iam_policy "warmasglambda_role_inline_policy" {
  name = "warmasglambda_role_inline_policy"
  policy = jsonencode({
  "Version": "2012-10-17",
  "Statement": [ {
      "Effect": "Allow",
...
resource "aws_iam_role_policy_attachment" "warmasglambda_role_attachment" {
  role = aws_iam_role.warmasglambda_role.name
  policy_arn = aws_iam_policy.warmasglambda_role_inline_policy.arn
}
...
resource "aws_cloudwatch_event_target" "eventbridge_launch_lambda_target" {
  ...
  arn       = aws_lambda_function.warmasglambda.arn
}
...
resource "aws_lambda_permission" "eventbridge_launch_invoke_lambda_permission" {
  ...
  function_name = "${aws_lambda_function.warmasglambda.arn}"
...
resource "aws_secretsmanager_secret_version" panorama_version {
  ...
  secret_string = jsonencode(local.lambda_secret_warmasg)
}
